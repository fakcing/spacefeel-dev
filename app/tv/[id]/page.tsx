...
export default function TvShowDetailPage() {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			try {
				...
				let details = await getMediaDetails(movieId, 'tv', language)
				if (!details) {
					// Если не нашлось, пробуем как фильм (на всякий случай)
					details = await getMediaDetails(movieId, 'movie', language)
				}
				setMovie(details)
			} catch (error) {
	...
	// Явно указываем 'tv', так как это страница сериалов
	const mediaType = 'tv'
									...
									{movie.credits.cast.slice(0, 12).map(person => (
										...
										>
											<div className='mx-auto mb-2 h-20 w-20 overflow-hidden rounded-full bg-primary-light'>
												{person.profile_path ? (
													<img
														/* FIX: Используем 'w500' чтобы убрать ошибку типов */
